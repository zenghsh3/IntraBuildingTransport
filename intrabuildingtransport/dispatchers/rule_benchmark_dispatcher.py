...
class RuleBenchmarkDispatcher(DispatcherBase):
  ...
  def policy(self, state):
    ...
    get_down_lift = []
    ele_is_stopped = []
    ret_actions = [ElevatorAction(-1, 1) for i in range(self._mansion._elevator_number)]
    ...
    for floor in state.RequiringUpwardFloors:
      upward_floor_address_dict[floor] = (-1, -HUGE)  # Addressing Elevator, Priority
    ...
    while not idle_ele_queue.empty():
        ...
        if(not assigned):
          if(len(state.RequiringDownwardFloors) > 0):
            ...
            if(max_unassigned_down_floor >= 0):
              ret_actions[sel_ele] = ElevatorAction(max_unassigned_down_floor, -1)
      ...
      if(state.ElevatorStates[sel_ele].Direction < 0):
        ...
        # In case no floor is assigned to the current elevator, we search all requiring-upward floor, find the lowest floor and assign it to the elevator
        if(not assigned):
      ...
      if(state.ElevatorStates[sel_ele].Direction == 0):
        ...
        for upward_floor in state.RequiringUpwardFloors:
          priority = -abs(upward_floor - state.ElevatorStates[sel_ele].Floor)
          if(priority > upward_floor_address_dict[upward_floor][1] and priority > sel_priority):
            sel_priority = priority
            sel_direction = 1
            sel_floor = upward_floor
        ...
        for downward_floor in state.RequiringDownwardFloors:
          priority = -abs(downward_floor - state.ElevatorStates[sel_ele].Floor)
          if(priority > downward_floor_address_dict[downward_floor][1] and priority > sel_priority):
            sel_priority = priority
            sel_direction = - 1
            sel_floor = downward_floor
        ...
        if(sel_floor > 0):
          ret_actions[sel_ele] = ElevatorAction(sel_floor, sel_direction)
          if(sel_direction > 0):
            if(upward_floor_address_dict[sel_floor][0] >= 0):
              idle_ele_queue.put(upward_floor_address_dict[sel_floor][0])
            upward_floor_address_dict[sel_floor] = (sel_ele, sel_priority)
