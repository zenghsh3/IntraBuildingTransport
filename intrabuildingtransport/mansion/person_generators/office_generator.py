...
class OfficePersonGenerator(PersonGeneratorBase):
  ...
  def configure(self, configuration):
    ...
    self._random_interval = list(map(lambda x: 3600.0 * self._particle_number / max(float(x), EPSILON), 
      self._config['RandomFreqPattern'].split(',')))
    self._upstairs_interval = list(map(lambda x: 3600.0 * self._particle_number / max(float(x), EPSILON),
      self._config['UpstairsFreqPattern'].split(',')))
    self._downstairs_interval = list(map(lambda x: 3600.0 * self._particle_number / max(float(x), EPSILON),
      self._config['DownstairsFreqPattern'].split(',')))
  ...
  def generate_person(self):
    ...
    for i in range(self._particle_number):
      if(random.random() < time_interval / self._random_interval[cur_hour]):
        ...
        ret_persons.append(PersonType(self._cur_id, random_weight, random_source_floor, random_target_floor, self._config._current_time))
        self._cur_id += 1
      ...
      if(random.random() < time_interval / self._upstairs_interval[cur_hour]):
        ...
        ret_persons.append(PersonType(self._cur_id, random_weight, source_floor, target_floor, self._config._current_time))
        self._cur_id += 1
