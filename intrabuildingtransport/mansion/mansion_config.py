...
class MansionConfig(object):
  ...
  def log_debug(self, string, *args):
    if(self._logger_level < 2):
      self._logger_stream_std.write(time.strftime('%Y-%m-%d\t%H:%M:%S',time.localtime(time.time())))
      self._logger_stream_std.write("\tDebug\t")
      if(self._add_simulation_time_in_log):
        self._logger_stream_std.write("world_time:%s\t"%simulation_time_to_str(self.world_time))
  ...
  def log_warning(self, string, *args):
    self._logger_stream_err.write(time.strftime('%Y-%m-%d\t%H:%M:%S',time.localtime(time.time())))
    self._logger_stream_err.write("\tWarning\t")
    if(self._add_simulation_time_in_log):
      self._logger_stream_err.write("world_time:%s\t"%simulation_time_to_str(self.world_time))
