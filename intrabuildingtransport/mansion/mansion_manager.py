...
class MansionManager(object):
  ...
  def reset_env(self):
    ...
    self._wait_upward_persons_queue = [collections.deque() for i in range(self._floor_number)]
    self._wait_downward_persons_queue = [collections.deque() for i in range(self._floor_number)]
    self._delivered_person = 0
    self._generated_person = 0
    self._abandoned_person = 0
  ...
  def run_mansion(self, actions):
    ...
    for idx in range(self._elevator_number):
      ...
      self._delivered_person += len(delivered_person_time)
    ...
    for ele_idx in ele_idxes:
      ...
      if(is_ready and is_open):
        self._config.log_debug("Floor: %d, Elevator: %s is open, %d persons are waiting to go upward, %d downward", 
           floor, self._elevators[ele_idx].name, 
           len(self._wait_upward_persons_queue[floor_idx]), len(self._wait_downward_persons_queue[floor_idx])
           )
        ...
        if(self._elevators[ele_idx]._direction == -1):
          for i in range(len(self._wait_downward_persons_queue[floor_idx])-1, -1, -1):
            ...
            if(req_succ):
              ...
              self._config.log_notice("Person %s is walking into the %s elevator", entering_person, self._elevators[ele_idx].name)
    ...
    for floor_idx in range(self._floor_number):
      ...
      for pop_idx in range(len(self._wait_downward_persons_queue[floor_idx]) - 1, -1, -1):
        if(self._config.raw_time - self._wait_downward_persons_queue[floor_idx][pop_idx].AppearTime > self._given_up_time_limit):
          ...
          break
    self._abandoned_person += give_up_persons
    
    cumulative_waiting_time = 0
    for i in range(self._floor_number):
      cumulative_waiting_time += self._dt * len(self._wait_upward_persons_queue[i])
      cumulative_waiting_time += self._dt * len(self._wait_downward_persons_queue[i])
    cumulative_waiting_time += loaded_person_num * self._dt

    return cumulative_waiting_time, sum(energy_consumption), give_up_persons
