...
PersonType = collections.namedtuple('Person',['ID', 'Weight', 'SourceFloor', 'TargetFloor', 'AppearTime'])
...
ElevatorAction = collections.namedtuple("Action", ["TargetFloor", "DirectionIndicator"])
...
def simulation_time_to_str(time):
  ...
  return "(Day: %d, Hour: %d, Minute: %d, Second: %2.2f)"%(time.Day, time.Hour, time.Min, time.Sec)
...
def velocity_planner(start_v, target_x, max_acc, max_spd, dt):
  ...
  reaction_dist_e = 0.5 * tmp_vel_e * abs(tmp_vel_e) / max_acc + 0.5 * (tmp_vel_e + start_v) * dt
  ...
  if((sign_x > 0 and target_x > reaction_dist_e-0.1*EPSILON) or
    ...
    des_vel = start_v
    while iterate < 5:
      tmp_vel = clip_value(start_v + sign_x * fraction_m * max_acc * dt, max_spd)
      reaction_dist = 0.5 * tmp_vel * abs(tmp_vel) / max_acc + 0.5 * (tmp_vel + start_v) * dt
    ...
    if(abs(target_x) < 0.1 * EPSILON):
      ...
      req_acc = clip_value(- 0.5 * abs(start_v * start_v / target_x) * sign_v, max_acc)
