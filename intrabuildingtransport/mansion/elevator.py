...
class Elevator(object):
  ...
  def __init__(self, 
    ...
    # User clicked buttons, if clicked, never deactivate before arriving at that floor!
    self._clicked_buttons = set()
    # Target of dispatcher, -1 if no target needs to be specified
    self._dispatch_target = -1
    ...
    # Predefined direction of the elvator:  0 - stop; 1 - Go Up; 2 - Go Down
    self._direction = 0 

    self._name = name
    self._is_overloaded_alarm = 0.0

    self._reaction_distance = 0.5 * (self._maximum_speed * self._maximum_speed) / self._maximum_acceleration
  ...
  def __repr__(self):
    return ("Elevator Object: %s\nState\n\tPosition: %f\n\tFloors: %2.2f\n\tVelocity: %f\n\tLoad: %f" + 
        ...
        "\n\tIs Door Opening: %d\n\tIs Door Closing: %d\n\tLoaded Persons: %s\n\tEntering Persons: %s\n\tExiting Persons: %s\n\t")%(self._name,
            self._current_position, self._current_position/self._floor_height + 1.0, 
            self._current_velocity, self._load_weight, self._direction, 
            self._dispatch_target, self._dispatch_target_direction, self._target_floors, self._clicked_buttons,
            self._is_overloaded_alarm, self._door_open_rate, self._is_door_opening, self._is_door_closing, self._loaded_person, self._entering_person, self._exiting_person)
  ...
  def state(self):
    ...
    return ElevatorState(floor, self._number_of_floors, 
        self._current_velocity, self._maximum_speed, self._direction, self._door_open_rate,
        self._dispatch_target, self._dispatch_target_direction,
        self._load_weight, self._maximum_capacity, self._target_floors,
        self._is_overloaded_alarm, self._is_door_opening, self._is_door_closing)
  ...
  def nearest_reachable_floor(self):
    ...
    reaction_distance = 0.5 * self._current_velocity * self._current_velocity / self._maximum_acceleration 
  ...
  def _check_abnormal_state(self):
    ...
    if(cur_floor < 1 - EPSILON or cur_floor > self._number_of_floors + EPSILON):
      self._config.log_fatal("Abnormal State detected for elevator %s, current_floor = %f", self._name, cur_floor)
    if(abs(self._current_velocity) > self._maximum_speed):
      self._config.log_fatal("Abnormal State detected for elevator %s, current_velocity = %f exceeds the maximum velocity %f", 
          self._name, self._current_velocity, self._maximum_speed)
    if(self._load_weight > self._maximum_capacity):
      self._config.log_fatal("Abnormal State detected for elevator %s, load_weight(%f) > maximum capacity (%f)",
    ...
    if(len(self._target_floors) > 0):
      for i in range(len(self._target_floors) - 1):
        if(self._direction * (self._target_floors[i + 1] - self._target_floors[i]) < - EPSILON):
          self._config.log_fatal("Abnormal State detected for elevator %s, direction(%d) do not match target floors %s",
              self._name, self._direction, self._target_floors)
      if(self._direction * (self._target_floors[0] - cur_floor) < - EPSILON):
        self._config.log_fatal("Abnormal State detected for elevator %s, direction(%d) do not match target floors %s and current_floor %2.2f, %s",
          self._name, self._direction, self._target_floors, cur_floor, self)
    if(self._direction * self._current_velocity < - 1.0):
      self._config.log_fatal("Abnormal State detected for elevator %s, direction(%d) dot not match current velocity %f, Elevator state: %s",
        self._name, self._direction, self._current_velocity, self)
  ...
  def _check_target_validity(self, dispatch_target):
    ...
    '''
    # 0: dispatch to the nearest reachable floor if no target, -1: no action, keep going
    ...
    #     self._maximum_acceleration + self._dt * 0.5 * abs(exp_v + self._current_velocity))
    reaction_distance = 0.5 * self._current_velocity * self._current_velocity / self._maximum_acceleration
  ...
  def _insert_target(self, new_target):
    ...
    if(self._direction >= 0):
      ...
      if(len(self._target_floors) < 1):
        ...
        #Else, find a position to insert, such that target floors are in ascending order
      ...
      if(len(self._target_floors) < 1):
        ...
        #Else, find a position to insert, such that target floors are in descending order
        sel_i = len(self._target_floors)
        for i in range(len(self._target_floors)):
          if(self._target_floors[i] == new_target):
            return
  ...
  def run_elevator(self):
    ...
    if(target_floor <= 0):
      ...
      target_offset = (target_floor - 1) * self._floor_height - self._current_position
    ...
    #if no target floors and current speeds = 0 and the door is closing, set direction = 0
    ...
    #Reaching the bottom or top of the floor, change the direction instantly
    ...
    if(target_floor > 0):
      ...
      delta_floor = float(target_floor) - 1.0 - self._current_position / self._floor_height
      if((delta_floor * self._direction) < - EPSILON):
        self._config.log_warning("Elevator: %s, Specified target floor is not achievable"+
            ", delta_floor = %f, neglected floor = %d, information: %s",
            self._name, delta_floor, target_floor, self)
        if(target_floor in self._target_floors):
          self._target_floors.remove(target_floor)
        
    #release the control command to the elevator door TODO: when would this case happen?
    if(abs(self._current_velocity) > EPSILON):
      if(self._is_door_opening == True):
        self._is_door_opening = False
    ...
    if(self._is_door_opening):
      self._door_open_rate = min(1.0, self._door_open_rate + self._door_open_close_velocity)
    elif(self._is_door_closing):
      self._door_open_rate = max(0.0, self._door_open_rate - self._door_open_close_velocity)

    #in case the door is open, print some log
    if(self._door_open_rate < EPSILON and self._is_door_closing):
      self._config.log_notice("Elevator: %s, Door is fully closed, Elevator at %2.2f floor",
          self._name, self._current_position/self._floor_height + 1.0)
    if(self._door_open_rate > 1.0 - EPSILON and self._is_door_opening):
      self._config.log_notice("Elevator: %s, Door is fully opening, Elevator at %2.2f floor",
          self._name, self._current_position/self._floor_height + 1.0)
    ...
    if(hold_elevator):
      if(abs(self._current_velocity) < self._dt * self._maximum_acceleration):
        tmp_velocity = 0.0
        eff_dt = abs(self._current_velocity) / self._maximum_acceleration
      else:
      ...
      tmp_velocity, eff_dt = velocity_planner(self._current_velocity, target_offset, 
          self._maximum_acceleration, self._maximum_speed, self._dt)
    ...
    self._current_position += 0.5 * (tmp_velocity + self._current_velocity) * eff_dt + tmp_velocity * (self._dt - eff_dt)
    #update the elevator velocity
    self._current_velocity = tmp_velocity
    #calculate the true acceleration
    acceleration = (tmp_velocity - self._current_velocity) / max(eff_dt, EPSILON)

    force_1 = (self._config.net_weight + self._load_weight) * (GRAVITY + acceleration)
    force_2 = self._config.rated_load * (GRAVITY - acceleration)
    net_force = force_1 - force_2
    m_load = abs(net_force) * self._config.pulley_radius / self._config.motor_gear_ratio / self._config.gear_efficiency
    energy_consumption = (m_load * abs(self._current_velocity) * self._config.gear_efficiency / self._config.motor_efficiency * eff_dt
        + self._config.standby_power_consumption * self._dt)
    if(self._is_door_opening or self._is_door_closing):
      energy_consumption += self._config.automatic_door_power * self._dt

    self._is_overloaded_alarm = max(0.0, self._is_overloaded_alarm - self._dt)
    ...
    for i in sorted(remove_tmp_idx, reverse=True):
      ...
      person_statistics.append(self._config.raw_time - out_person.AppearTime)
      ...
      if(abs(delta_distance) < EPSILON):
        if(len(self._loaded_person[floor_idx]) > 0):
          if(len(self._exiting_person) < self._config._mpee_number):
            ...
            self._config.log_notice("Person %s is walking out of the %s elevator", tmp_unload_person, self._name)
    ...
    return energy_consumption, person_statistics, sum(len(row) for row in self._loaded_person)
  ...
  def require_door_closing(self):
    ...
    if(self._door_open_rate > EPSILON and not self._is_door_opening
      ...
      self._config.log_debug("Require door closing failed, door_open_rate = %f, is_unloading = %d, entering_person = %s",
        self._door_open_rate, self._is_unloading, self._entering_person)
  ...
  def person_request_in(self, person):
    ...
    if(abs(person.SourceFloor - cur_floor) > EPSILON or
      ...
      self._config.log_debug("Refuse to accommadate person: elevator: %s, person: %s, illegal request", self, person)
      return False
    if(len(self._entering_person) >= self._mpee_number):
      self._config.log_debug("Refuse to accommadate person: elevator: %s, person: %s, maximum parallel stream arrived", self, person)
      return False
    # if(self._is_overloaded_alarm > EPSILON):
